...
class series_scale(nn.Module):  # x: [Batch, Input length, Channel]
    ...
    def forward(self, x):
        ...
        moving_std = torch.clamp(moving_std, min=0.01)
        seq_scaled = (x - moving_mean) / moving_std
        return seq_scaled, moving_mean, moving_std
...
class model_backbone(nn.Module):
    def __init__(
        ...
        self.h = h
        self.scaler = series_scale(decomp_kernel_size)
        self.transformer_input_size = transformer_input_size

        self.linearEncoder = nn.Linear(input_size, h)
        self.output_layer = MLPStack(
            input_dim=3*h, hidden_dims=[transformer_hidden_size,transformer_hidden_size] * decoder_layers, output_dim=h
    ...
    def forward(self, windows_batch):
        ...
        ### std:
        z_scaled, mean, var = self.scaler(z.permute(0, 2, 1))
        z_scaled, mean, var = z_scaled.permute(0, 2, 1), mean.permute(0, 2, 1), var.permute(0, 2, 1)   # [Batch, 1, input_size]
        # plt.plot(z_scaled[0][0].cpu(),label = 'x_scaled')
        # plt.plot(mean[0][0].cpu(),label = 'mean')
        # plt.plot(var[0][0].cpu(),label = 'var')
        # plt.legend()
        # plt.show()
        ...
        if self.is_pretrain:
            ...
            # truncate
            z = z_scaled[:, :, -self.transformer_input_size :]
            # do patching
        ...
        # fine-tune阶段 
            ...
            z_var = z_var.reshape(z_.shape[0], -1)  # （bs,pred_len)
            z_mean = z_mean.reshape(z_.shape[0], -1)  # （bs,pred_len)
