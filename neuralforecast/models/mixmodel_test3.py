...
class ResidualBlock(nn.Module):
    def __init__(
        self, hidden_size, input_size, output_size, dropout, layer_norm=True
    ) -> None:
        super().__init__()
...
class mlp_encoder(nn.Module):
    ...
    def forward(
        self, trend_init, hist_exog, stat_exog, futr_exog, insample_mask
    ) -> torch.tensor:
        z = trend_init.reshape(trend_init.shape[0], -1) 
        z = self.encoder(z)
...
class MIXModel(BaseWindows):
    ...
    def __init__(
        ...
        **trainer_kwargs,
        ...
        patch_num = int((transformer_input_size - former_patch_len) / former_stride + 1)
        self.temporal_decoder = ResidualBlock(
            input_size=patch_num*former_hidden_size+mlp_hidden_size,
            output_size=h,
    ...
    def forward(self, windows_batch):  # x: [batch, input_size]
        ...
        z_t = self.mlp_encoder(
            trend_init, hist_exog, stat_exog, futr_exog, insample_mask
        ) # z_t: [bs, d_t]

        ### truncate then local pattern:
        z_s = self.tst_encoder(seasonal_init[:, :, -self.transformer_input_size :])   #z_s: [bs,, d_s] , ds = patch_num * former_hidden_size
        
        ### concat then docode:
        y = self.temporal_decoder(torch.cat((z_s, z_t), dim=1))
        y = y.unsqueeze(1)
...
### 编码解码清晰版本, mlp encoder和 decoder为 TiDE block叠加得到的 stack，训练速度很快，性能不够好
