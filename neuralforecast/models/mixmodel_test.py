...
class NHITSBlock(nn.Module):
    ...
    def __init__(
        ...
        self.input_layer = nn.Linear(
            in_features=input_size, out_features=mlp_units[0][0]
        )
        hidden_layers = []
        ...
        self.output_layer = nn.Linear(
            in_features=mlp_units[-1][-1], out_features=n_theta
        )
        self.basis = basis
    ...
    def forward(
        ...
        insample_y = self.input_layer(insample_y)
        insample_y = self.layers(insample_y)
...
class mlp_encoder(nn.Module):
    ...
    def __init__(
        ...
        n_blocks: list = [1] * 3, 
        block_units: list = [[512, 512]] * 3,  # 每一个block的层数和每一层的神经元个数
    ...
    def forward(self, trend_init, hist_exog, stat_exog, futr_exog, insample_mask):
        z = trend_init  # z: [bs x nvars x seq_len]
...
class MIXModel(BaseWindows):
    ...
    def __init__(
        ...
        ### mlp_encoder：
        mlp_layers: int = 3,  # 几个stack
        stack_blocks: int = 3,   # 每个stack有几个block
        block_hidden_size: int = 512,
        stack_pool_kernel_size: int = 2,
        stack_freq_downsample: list = [4, 2, 1],
        ...
        **trainer_kwargs,
        ...
        stack_types = ['identity'] * mlp_layers
        n_blocks: list = [stack_blocks] * mlp_layers
        stack_pool_kernel_size: list = [stack_pool_kernel_size] * (mlp_layers - 1) + [1]
        block_layers: int = 1
        block_units: list = [[block_hidden_size, block_hidden_size]] * block_layers
        ...
        self.mlp_encoder = mlp_encoder(
            ...
            stat_exog_list=stat_exog_list,
            stack_types=stack_types,
            n_pool_kernel_size=stack_pool_kernel_size,
            n_freq_downsample=stack_freq_downsample,
            n_blocks=n_blocks,
    ...
    def forward(self, windows_batch):  # x: [batch, input_size]
        ...
        x = insample_y.unsqueeze(1)  # [bs,1,L]
 
        ### decomp:  
        seasonal_init, trend_init = self.decompsition(x.permute(0, 2, 1))
        seasonal_init, trend_init = seasonal_init.permute(0, 2, 1), trend_init.permute(0, 2, 1)  # [Batch, 1, input_size]

        ### trend and exog:
        z_t = self.mlp_encoder(trend_init, hist_exog, stat_exog, futr_exog, insample_mask)

        ### truncate then local pattern:
        z_s = self.tst_encoder(seasonal_init[:, :, -self.transformer_input_size :])

        ### concat then docode:
        z_s = z_s.reshape(z_s.shape[0], -1)  # （bs,pred_len)
        z_t = z_t.reshape(z_t.shape[0], -1)  # （bs,pred_len)
        y = self.output_layer(torch.cat((z_s, z_t), dim=1))
        y = y.unsqueeze(1)

        ### Linear residual:
        y = y + self.linearResidual(x)
    
        # Domain map
        y = y.reshape(y.shape[0], self.h, -1)  # x: [Batch, h, c_out]
