...
class model_backbone(nn.Module):
    def __init__(
        ...
        # Fourier
        self.fourier = SpectralConv1d(c_in,c_out,input_size,int(input_size*fourier_sample_rate))
    ...
    def forward(self, windows_batch):
        ...
        # remove noise
        z = self.fourier(z.reshape(z.shape[0], z.shape[1], z.shape[2], 1).permute(0, 1, 3, 2))
...
class SpectralConv1d(nn.Module):
    def __init__(self, in_channels:int, out_channels:int,seq_len:int, modes1:int,compression=0,ratio=0.5,mode_type=0):
