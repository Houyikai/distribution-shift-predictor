...
class decoder(nn.Module):
    def __init__(self, input_size, h, patch_num, d_t, d_s, dropout):
        ...
        input_size = h * d_t + patch_num * d_s 
...
class HEncoder(BaseWindows):
    ...
    def forward(self, windows_batch):  # x: [batch, input_size]
        ...
        ### trend and exog:
        z_t = self.mlp_encoder(x, hist_exog, stat_exog, futr_exog, insample_mask)  
        # z_t = z_t.permute(0,1,3,2)
     

        ### truncate then local pattern: 
        z_s = self.tst_encoder(x[:, :, -self.transformer_input_size :])

        ### concat then docode:
        y = self.output_layer(z_t, z_s)
        y = y.unsqueeze(1)
        ...
        # if self.testflag:
        #     print('input ',insample_y.shape)
        #     print(z_t.shape, z_s.shape)
        #     plt.plot(z_t[0][-1].cpu().numpy())
        #     plt.show()
        #     plt.plot(z_s[0][-1].flatten().cpu().numpy())
        #     plt.show()
        #     # plt.plot(insample_y[-1].cpu().numpy())
        #     plt.plot(forecast[-1].cpu().numpy())
        #     self.testflag = False
        return forecast
