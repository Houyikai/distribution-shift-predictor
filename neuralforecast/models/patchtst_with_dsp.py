...
from neuralforecast.models.fedformer import FourierBlock
...
class PatchTST(BaseWindows):
    ...
    **References:**<br>
    ...
    def __init__(
        ...
        **trainer_kwargs
        ...
        self.model = PatchTST_backbone(
            c_in=c_in,
            c_out=c_out,
            input_size=self.transformer_input_size,
            h=h,
            patch_len=patch_len,
            stride=stride,
            max_seq_len=max_seq_len,
    ...
    def forward(self, windows_batch):  # x: [batch, input_size]
        ...
        x_norm, x_mean, x_std = x_norm.permute(0, 2, 1), x_mean.permute(0, 2, 1), x_std.permute(0, 2, 1)

        ### distribution shift prediction:
        mean_hat = self.dsp_mean(x_mean, insample_mask, hist_exog, stat_exog, futr_exog)
        std_hat = self.dsp_std(x_std, insample_mask, hist_exog, stat_exog, futr_exog)

        ### truncate: 
        x = self.model(x_norm[:, :, -self.transformer_input_size:])
