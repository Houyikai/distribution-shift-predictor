...
class series_scale(nn.Module):  # x: [Batch, Input length, Channel]
    """
    Series scale block
    """

    def __init__(self, kernel_size, stride:int = 1):
        super(series_scale, self).__init__()
        self.moving_avg = moving_avg(kernel_size, stride)
        self.moving_std = moving_std(kernel_size, stride)

    def forward(self, x):
        moving_mean = self.moving_avg(x)
        moving_std = self.moving_std(x)
        moving_std = torch.clamp(moving_std, min=0.001)
        x_scaled = (x - moving_mean) / moving_std
        return x_scaled, moving_mean, moving_std
...
class model_backbone(nn.Module):
    def __init__(
        ...
        self.h = h
        self.decompsition = series_scale(decomp_kernel_size)
        self.transformer_input_size = transformer_input_size

        # self.linear_residual = nn.Linear(input_size, h)
        # self.output_layer = MLPStack(
        #     input_dim=h + h + h, hidden_dims=[transformer_hidden_size,transformer_hidden_size] * decoder_layers, output_dim=h
        # )  
        ...
        self.blocks_mean = torch.nn.ModuleList(blocks)
        ...
        self.blocks_var = torch.nn.ModuleList(blocks)
    ...
    def forward(self, windows_batch):
        ...
        x = insample_y.unsqueeze(1)  # x: [Batch, 1, input_size]
        # norm
        if self.revin:
            x = x.permute(0, 2, 1)
        ...
        ### decomp:
        x_std, x_mean, x_var = self.decompsition(x.permute(0, 2, 1))
        x_std, x_mean, x_var = x_std.permute(0, 2, 1), x_mean.permute(0, 2, 1), x_var.permute(0, 2, 1)    
        # print(x_std.shape,x_mean.shape,x_var.shape)  # [Batch, nvars, input_size]
        
        # 预训练阶段
        if self.is_pretrain:
            # truncate
            z = x_std[:, :, -self.transformer_input_size :]
            # do patching
        ...
        # fine-tune阶段 
            ...
            # for i, block in enumerate(self.blocks_mean):
            ...
            #     forecast = forecast + block_forecast
            # z_var = forecast
            # z_var = z_var.permute(0,2,1)

            # truncate
            # import matplotlib.pyplot as plt
            # if self.flag:
            #     plt.figure(figsize=(20,5))
            #     plt.plot(x[10][0].cpu(),label = 'x')
            #     plt.plot(x_std[10][0].cpu(),label = 'x_std')
            #     plt.plot(x_mean[10][0].cpu(),label = 'x_mean')
            #     plt.plot(x_var[10][0].cpu(),label = 'x_var')

            
            ###   mean:

            z_mean = x_mean[:, :, -self.transformer_input_size :]
            if self.padding_patch == "end":
                z_mean = self.padding_patch_layer(z_mean)
            z_mean = z_mean.unfold(
                dimension=-1, size=self.patch_len, step=self.stride
            )  # z: [bs x nvars x patch_num x patch_len]
            z_mean = z_mean.permute(0, 1, 3, 2)  # z: [bs x nvars x patch_len x patch_num]

            # transformer encoding
            z_mean = self.backbone(z_mean)
            z_mean = self.head(z_mean)


            ###   var:

            z_var = x_var[:, :, -self.transformer_input_size :]
            if self.padding_patch == "end":
                z_var = self.padding_patch_layer(z_var)
            z_var = z_var.unfold(
                dimension=-1, size=self.patch_len, step=self.stride
            )  # z: [bs x nvars x patch_num x patch_len]
            z_var = z_var.permute(0, 1, 3, 2)  # z: [bs x nvars x patch_len x patch_num]

            # transformer encoding
            z_var = self.backbone(z_var)
            z_var = self.head(z_var)
            ...
            if self.flag:
              ...
              fig.text(0.5, -0.1, 'input after MNK', ha='center', va='bottom', fontsize=18)
              plt.show()
            # do patching
            if self.padding_patch == "end":
                z = self.padding_patch_layer(z)
            ...
            if self.flag:
              ...
              fig.text(0.5, -0.1, 'attentionmap after MNK', ha='center', va='bottom', fontsize=18)
              
              # 设置全局字体大小
              # mpl.rcParams['font.size'] = 20  # 设置字体大小为12
              #  # 隐藏坐标轴
              # ax.set_xticks([])
              # ax.set_yticks([])
            ...
            z = self.head(z)  # z: [bs x nvars x h]
            # if self.flag:
            #     plt.plot(z[10][0].cpu(),label = 'z^')
            #     plt.plot(z_mean[10][0].cpu(),label = 'z_mean')
            #     plt.plot(z_var[10][0].cpu(),label = 'z_var')
            #     plt.legend()
            #     plt.show()
            #     self.flag = False
            ### concat then docode:
            ...
            z_var = z_var.reshape(z_var.shape[0], -1)  # （bs,pred_len)
            z_mean = z_mean.reshape(z_mean.shape[0], -1)  # （bs,pred_len)
            # z = self.output_layer(torch.cat((z, z_mean, z_var), dim=1))
            z = z * z_var + z_mean
            z = z.unsqueeze(1)

        ### Linear residual:
        # z = z + self.linear_residual(x)
