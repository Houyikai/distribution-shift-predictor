...
# lightingmodule 和 nn.module几乎没有区别，只是lightingmodule有一些额外的功能，比如自动保存模型，自动保存日志等等，以及一些可以实现的方法，比如training_step，validation_step等等
class BaseWindows(pl.LightningModule): 
    ...
    def __init__(
        ...
        **trainer_kwargs,
        ...
        # log 学习率
        trainer_kwargs["callbacks"] += [pl.callbacks.LearningRateMonitor(logging_interval='epoch')]
    ...
    """
       trainingstep是lightning 提供的可以实现的方法，用来代替原来的training loop;
       在这个模块里，我们要从batch中获取数据，进行forward，以及进行loss（目标函数）的计算，还包括log的记录
       这个函数接收batch,返回loss 
    """
    def training_step(self, batch, batch_idx):
        ...
        return {'loss': loss}  # 也可以这样写 'loss'是保留字
        # return loss
    ...
    def _compute_valid_loss(self, outsample_y, output, outsample_mask, temporal_cols):
        if self.loss.is_distribution_output: # 检查 self.loss 是否表示分布输出的损失函数
            _, y_loc, y_scale = self._inv_normalization(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(n_batches):
            ...
            # print('original_outsample_y',original_outsample_y)
    ...
    def fit(self, dataset, val_size=0, test_size=0, random_seed=None):
        ...
        **Parameters:**<br>
        ...
        print("Training model:", self.trainer_kwargs)
        trainer = pl.Trainer(**self.trainer_kwargs)
        trainer.fit(self, datamodule=datamodule)

    def test(self, dataset, test_size=0, random_seed=None):
    ...
    def test(
        ...
        **data_module_kwargs,
        ...
        trainer = pl.Trainer(**pred_trainer_kwargs)
        fcsts = trainer.test(self, datamodule=datamodule)
        fcsts = torch.vstack(fcsts).numpy().flatten()
        fcsts = fcsts.reshape(-1, len(self.loss.output_names))
        return fcsts
