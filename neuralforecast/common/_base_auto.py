    ...
    **Parameters:**<br>
    ...
    def fit(self, dataset, val_size=0, test_size=0, random_seed=None):
        ...
        **Returns:**<br>
        ...
        # best_config_dir = os.path.join(project_root, "best_config")
        # # 如果 best_config 目录不存在，则创建它
        # if not os.path.exists(best_config_dir):
        #     os.makedirs(best_config_dir)

        # `"wb"` 是文件打开模式，它指定了文件在打开时的操作方式。在这里：

        # - `"w"` 表示写入（write）模式，即打开文件以进行写入操作。
        # - `"b"` 表示二进制（binary）模式，即以二进制形式处理文件。

        # 组合起来，`"wb"` 表示以二进制写入模式打开文件，用于处理二进制数据，例如使用 `pickle` 序列化数据时。在这种模式下，文件将以二进制形式写入，而不是以文本形式写入。这对于保存二进制数据（如Python对象的序列化）非常重要，因为二进制数据在写入和读取时需要保持不变性。

        # 如果您要打开文本文件进行写入，可以使用 `"w"` 模式，例如 `open(text_file_path, "w")`。但是对于包含二进制数据的文件（如使用 `pickle` 保存的文件），必须使用 `"wb"` 模式以确保数据的完整性。


        # config_file_path = os.path.join(best_config_dir, "config.txt")
